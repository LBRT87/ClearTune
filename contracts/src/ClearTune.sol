...
contract ClearTune {
    ...
    uint256 public ratePerPlay = 20000;
    uint16 public platformFeeBps = 800;
    uint256 public minTopUp = 50000000;
    ...
    function topUp(uint256 amount) external {
        require(amount >= minTopUp, "ClearTune: below minimum top-up");
        require(token.transferFrom(msg.sender, address(this), amount), "ClearTune: transferFrom failed");
        balance[msg.sender] += amount;
        emit ToppedUp(msg.sender, amount, balance[msg.sender]);
    }
    ...
    function _settlePlay(address listener, uint256 songId) internal {
        ...
        if (balance[listener] < ratePerPlay) {
            emit PlaySkippedLowBalance(listener, songId);
            return;
        }

        balance[listener] -= ratePerPlay;
        uint256 fee = (ratePerPlay * platformFeeBps) / TOTAL_BPS;
        uint256 toPayees = ratePerPlay - fee;
        treasury += fee;
        _distribute(s, toPayees);

        emit PlayPaid(listener, songId, ratePerPlay);
    }
    ...
    function setConfig(uint256 newRatePerPlay, uint16 newPlatformFeeBps, uint256 newMinTopUp) external onlyOwner {
        ...
        emit ConfigUpdated(newRatePerPlay, newPlatformFeeBps, newMinTopUp);
    }

    function getConfig() external view returns (uint256, uint16, uint256) {
        return (ratePerPlay, platformFeeBps, minTopUp);
    }
