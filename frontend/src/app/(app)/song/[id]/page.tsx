  ...
  return (
    <main className="max-w-[1040px] mx-auto px-5 py-12">
      <span className="text-dim font-display text-[10px] block mb-4">
        SONG #{String(song.song_id_onchain).padStart(4, "0")}
      </span>
      ...
      <Card eyebrow="SPLIT SHEET — WHERE THE ROYALTIES GO" className="max-w-none">
        {splitSheet.length === 0 && (
          <p className="text-dim text-lg">Can&apos;t read from the contract yet (not deployed, or the song isn&apos;t registered on-chain).</p>
        )}
        {splitSheet.map((row) => (
